...
import { DataUserLoginResponseModel, LoginRequestModel, LoginResponseModel, SignupRequestModel, SignupResponseModel } from '../app.models';
...
export class AuthService extends BaseNetworkService {
  ...
  get isSuperAdmin() {
    return computed(() => this._user() !== null && this._user()?.user.role?.roleName === 'super_admin')
  }
  get isAdmin() {
    return computed(() => this._user() !== null && this._user()?.user.role?.roleName === 'admin')
  }
  get token() {
  ...
  signup(formData: SignupRequestModel) {
    ...
    return this.post<SignupRequestModel, SignupResponseModel>(url, formData, errorMessage)
  }
